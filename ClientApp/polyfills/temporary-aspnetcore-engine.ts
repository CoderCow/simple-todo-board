...
import { platformServer, platformDynamicServer, PlatformState, INITIAL_CONFIG, renderModuleFactory } from
...
export class FileLoader implements ResourceLoader {
  public get(url: string): Promise<string> {
    return new Promise((resolve, reject) => {
      fs.readFile(url,
        (err: NodeJS.ErrnoException, buffer: Buffer) => {
          if (err) {
            return reject(err);
          }
...
export function ngAspnetCoreEngine(
  ...
  html: string,
  globals: { styles: string, title: string, meta: string, transferData?: {}, [key: string]: any }
}> {
